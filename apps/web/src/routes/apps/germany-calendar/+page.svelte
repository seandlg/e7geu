<script lang="ts">
  ...
  const cacheKey = `germany-calendar-${firstYear}-${lastYear}`;
  ...
  let selectedStateCodes = $state<string[]>(['DE-BE']);
  ...
  const selectedStates = $derived(
    germanStates.filter((state) => selectedStateCodes.includes(state.code)),
  );
  const selectedStateSet = $derived(new Set(selectedStateCodes));
  const matchingStates = $derived(filterStates(germanStates, stateQuery));
  const days = $derived(monthGrid(year, month));
  const selectedSchoolHolidays = $derived(
    schoolHolidaysOn(schoolHolidays, selectedDate, selectedStateSet),
  );
  const selectedPublicHolidays = $derived(
    showPublicHolidays ? publicHolidaysOn(publicHolidays, selectedDate, selectedStateSet) : [],
  );
  const canShowOverlap = $derived(selectedStateCodes.length >= 2);
  ...
  onMount(() => {
    ...
    try {
      ...
      if (cached) {
        ...
        if (Array.isArray(data.school) && Array.isArray(data.public)) {
    ...
    fetchCalendarData(years, controller.signal)
      .then((data) => {
        schoolHolidays = data.school;
        publicHolidays = data.public;
        status = 'ready';
  ...
  function toggleState(code: string): void {
    selectedStateCodes = selectedStateCodes.includes(code)
      ? selectedStateCodes.filter((selected) => selected !== code)
      : [...selectedStateCodes, code];
    if (selectedStateCodes.length < 2) overlapOnly = false;
  }

  function schoolFor(date: string): SchoolHoliday[] {
    return schoolHolidaysOn(schoolHolidays, date, selectedStateSet);
  }

  function publicFor(date: string): PublicHoliday[] {
    return showPublicHolidays ? publicHolidaysOn(publicHolidays, date, selectedStateSet) : [];
  }

  function stateName(code: string): string {
    return germanStates.find((state) => state.code === code)?.name ?? code;
  }

  function stateShortCode(code: string): string {
    return germanStates.find((state) => state.code === code)?.shortCode ?? code;
  }

  function dayClass(inMonth: boolean, schoolCount: number, date: string): string {
  ...
  function publicScope(holiday: PublicHoliday): string {
    if (!holiday.subdivisionCodes || holiday.subdivisionCodes.length === 0) return 'Nationwide';
    const names = holiday.subdivisionCodes
      .filter((code) => selectedStateSet.has(code))
      .map(stateName);
...
<svelte:head>
  <title>Germany Calendar — e7g.eu</title>
  <meta
    name="description"
    content="Compare German school holidays and public holidays in a simple calendar."
  />
</svelte:head>
...
<main class="mx-auto min-h-dvh max-w-6xl px-3 pb-[max(3rem,env(safe-area-inset-bottom))] pt-[max(1.5rem,env(safe-area-inset-top))] sm:px-8 sm:pt-10">
  ...
  <section class="mt-8 grid gap-4 lg:grid-cols-[20rem_minmax(0,1fr)] lg:items-start">
    <aside class="glass-panel rounded-[1.75rem] p-4 sm:p-5" aria-labelledby="school-heading">
      <div class="flex items-start justify-between gap-3">
        <div>
          <p class="m-0 text-xs font-750 tracking-[0.15em] text-teal-300 uppercase">Schulferien</p>
          <h2 id="school-heading" class="mb-0 mt-1 text-xl font-750 tracking-tight text-white">Choose states</h2>
        </div>
      ...
      <label class="focus-within:ring-teal-300/30 mt-4 flex items-center gap-2.5 rounded-xl border border-white/10 bg-black/20 px-3.5 py-3 text-slate-500 transition focus-within:border-teal-300/30 focus-within:ring-3">
        ...
        <input
          ...
          aria-label="Search German states"
          autocomplete="off"
          bind:value={stateQuery}
        />
      </label>

      <div class="mt-3 max-h-64 space-y-1 overflow-y-auto pr-1" aria-label="German states">
        {#each matchingStates as state (state.code)}
          {@const active = selectedStateCodes.includes(state.code)}
          <button
            type="button"
          ...
          >
            ...
            <span class="min-w-0 flex-1 truncate text-sm font-650">{state.name}</span>
            <span class="text-[0.65rem] font-750 tracking-wider text-slate-600">{state.shortCode}</span>
          </button>
        {/each}
        {#if matchingStates.length === 0}
          <p class="m-0 px-3 py-5 text-center text-sm text-slate-600">No state found.</p>
        {/if}
      </div>

      {#if selectedStates.length > 0}
        <div class="mt-4 flex flex-wrap gap-1.5 border-t border-white/8 pt-4" aria-label="Selected states">
          {#each selectedStates as state (state.code)}
            <button
              type="button"
              ...
              onclick={() => toggleState(state.code)}>{state.shortCode} <span aria-hidden="true">×</span></button
            >
          {/each}
        </div>
      {/if}
      ...
      <div class="mt-4 space-y-2 border-t border-white/8 pt-4">
        <label class="flex cursor-pointer items-center justify-between gap-4 rounded-xl px-1 py-2 text-sm font-650 text-slate-300">
          <span>
            Public holidays
            <span class="mt-0.5 block text-xs font-450 text-slate-600">Feiertage for your states</span>
          </span>
        ...
        <label class={`flex items-center justify-between gap-4 rounded-xl px-1 py-2 text-sm font-650 ${canShowOverlap ? 'cursor-pointer text-slate-300' : 'cursor-not-allowed text-slate-600'}`}>
          <span>
            Overlap only
            <span class="mt-0.5 block text-xs font-450 text-slate-600">Select at least two states</span>
          </span>
    ...
    <div class="min-w-0">
      <section class="glass-panel overflow-hidden rounded-[1.75rem]" aria-labelledby="month-heading">
        <div class="flex items-center justify-between gap-3 border-b border-white/8 px-3 py-3 sm:px-5 sm:py-4">
          <button
            type="button"
            class="focus-ring grid h-10 w-10 shrink-0 cursor-pointer place-items-center rounded-full border border-white/10 bg-white/[0.04] text-xl text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-25"
            aria-label="Previous month"
            disabled={year === firstYear && month === 0}
            onclick={previousMonth}>‹</button
          >
          <div class="text-center">
            <h2 id="month-heading" class="m-0 text-lg font-780 tracking-tight text-white sm:text-2xl">{displayMonth()}</h2>
          ...
          <button
            ...
            disabled={year === lastYear && month === 11}
        ...
        {#if status === 'loading'}
          <div class="grid min-h-96 place-items-center p-8" aria-live="polite">
            <div class="text-center">
              ...
              <p class="mb-0 mt-4 text-sm text-slate-500">Loading holidays through {lastYear}…</p>
            </div>
          </div>
        {:else if status === 'error'}
          <div class="grid min-h-96 place-items-center p-8 text-center" aria-live="polite">
            <div>
              ...
              <p class="mb-0 mt-2 max-w-sm text-sm leading-6 text-slate-500">Check your connection and reload the page. The calendar itself stays on your device.</p>
            </div>
          </div>
        {:else}
          <div class="p-1.5 sm:p-4">
            ...
            <div class="grid grid-cols-7 gap-px overflow-hidden rounded-xl bg-white/6" role="grid" aria-label={displayMonth()}>
              {#each days as day (day.date)}
                {@const daySchool = schoolFor(day.date)}
                {@const stateCodes = overlappingStateCodes(daySchool)}
                {@const dayPublic = publicFor(day.date)}
                {@const visibleSchool = !overlapOnly ? stateCodes.length > 0 : stateCodes.length >= 2}
                <button
                  type="button"
                  class={`focus-ring relative min-h-14 min-w-0 cursor-pointer border-0 p-1.5 text-left transition sm:min-h-23 sm:p-2.5 ${dayClass(day.inMonth, stateCodes.length, day.date)}`}
                  role="gridcell"
                  aria-label={`${displayDate(day.date)}${stateCodes.length ? `, school holidays in ${stateCodes.length} selected states` : ''}${dayPublic.length ? `, ${dayPublic.map((holiday) => holiday.name).join(', ')}` : ''}`}
                ...
                >
                  ...
                  {#if visibleSchool}
                    <span class={`absolute bottom-1.5 left-1.5 max-w-[calc(100%-0.75rem)] truncate rounded-md px-1.5 py-0.5 text-[0.54rem] font-800 leading-3 sm:bottom-2 sm:left-2 sm:text-[0.65rem] ${overlapOnly ? 'bg-violet-300 text-violet-950' : 'bg-teal-300/18 text-teal-200'}`}>
                      {stateCodes.length === 1 ? stateShortCode(stateCodes[0]) : `${stateCodes.length} states`}
                    </span>
                  {/if}
      ...
      <section class="mt-4 rounded-[1.5rem] border border-white/8 bg-white/[0.025] p-4 sm:p-5" aria-live="polite" aria-labelledby="day-heading">
        ...
        {:else}
          <div class="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            {/each}
            {#each selectedSchoolHolidays as holiday (`${holiday.id}-${holiday.stateCode}`)}
              <div class="rounded-xl border border-teal-300/12 bg-teal-300/[0.045] px-3.5 py-3">
                ...
                <p class="mb-0 mt-1 text-xs text-teal-200/45">{stateName(holiday.stateCode)}</p>
              </div>
            {/each}
          </div>
        {/if}
  ...
  <footer class="mt-6 px-2 text-center text-xs leading-5 text-slate-600 sm:px-0">
    ...
    Regional and local exceptions can apply; check official state announcements when it matters.
  </footer>
</main>
