...
<style>
  .cv-app { display: flex; min-height: 100dvh; flex-direction: column; }
  ...
  .file-actions button, .mobile-tabs button { cursor: pointer; border: 1px solid rgb(255 255 255 / 0.1); border-radius: 10px; background: rgb(255 255 255 / 0.055); padding: 8px 11px; color: #cbd5e1; font: inherit; font-size: 0.75rem; font-weight: 700; }
  .file-actions button:hover { background: rgb(255 255 255 / 0.1); color: white; }
  ...
  .new-menu > div { position: absolute; top: calc(100% + 5px); right: 0; z-index: 20; display: none; min-width: 130px; border: 1px solid rgb(255 255 255 / 0.12); border-radius: 11px; background: #0f1b2e; padding: 5px; box-shadow: 0 16px 40px rgb(0 0 0 / 0.4); }
  .new-menu:hover > div, .new-menu:focus-within > div { display: grid; }
  .new-menu > div button { border: 0; text-align: left; }
  .workspace-grid { display: grid; min-height: 0; flex: 1; grid-template-columns: minmax(320px, 430px) minmax(0, 1fr); }
  .editor-shell { overflow: auto; border-right: 1px solid rgb(255 255 255 / 0.09); background: rgb(7 17 31 / 0.72); }
  .file-note { margin: 4px 22px 24px; color: #475569; font-size: 0.68rem; line-height: 1.5; }
  .preview-shell { overflow: auto; background: #182235; padding: 20px 20px 48px; }
  ...
  @media print {
    ...
    .cv-app { display: block; min-height: 0; }
